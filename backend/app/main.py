...
# DB
DATABASE_URL = "sqlite:///data/events.db"  # fixed path; DB will be in backend/data/
...
async def login_post(
    ...
    event.command = "web_login attempt"
...
async def dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    ssh_count = 14  # baseline from your note
    cowrie_txt = os.path.join(
        os.path.dirname(__file__), "..", "cowrie-parsed.txt"
    )
