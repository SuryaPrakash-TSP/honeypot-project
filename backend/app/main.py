...
def ensure_event_columns():
    """Ensure DB schema has required production columns."""
...
async def lifespan(app_: FastAPI):
    ...
    if rf_model_path.exists() and rf_metadata_path.exists():
        try:
            ...
            app_state["rf_features"] = rf_metadata.get(
            ...
            print(
                f"Loaded RF: {app_state['rf_info']['version']} | "
                f"{app_state['rf_info']['accuracy']:.1%}"
            )
    ...
    app_state["hybrid_info"] = "Adaptive Fusion Active"
...
def get_ip_freq(db: Session, ip: str) -> float:
    result = db.execute(
        text("SELECT COUNT(*) FROM events WHERE source_ip = :ip"),
        {"ip": ip}
    ).scalar()
...
async def root():
    ...
    return {
        "status": "Phase 9 Complete - Hybrid RF+LSTM Live",
        "events": count,
        "models": {
            "rf": f"{rf_info.get('version', 'N/A')} | {rf_info.get('accuracy', 0):.1%}"
            if rf_info else "Disabled",
            "lstm": f"{lstm_info.get('version', 'N/A')} | {lstm_info.get('accuracy', 0):.1%}"
            if lstm_info else "Disabled",
        },
    }
...
async def model_info():
    ...
    return {
        ...
        "status": "Dual models loaded" if rf_info and lstm_info else "Partial models loaded",
...
async def predict_hybrid_endpoint(request: dict):
    ...
    return {
        **hybrid_result,
        ...
        "model": "Hybrid RF v7.1 + LSTM v8.1",
    }
...
async def dashboard_view(request: Request, db: Session = Depends(get_db)):
    ...
    rf_info = app_state.get("rf_info", {})
    lstm_info = app_state.get("lstm_info", {})
    hybrid_info = app_state.get("hybrid_info", "Adaptive Fusion Active")
    ...
    return templates.TemplateResponse(
        "dashboard.html",
        {
            ...
            "phase": "Phase 9 Complete - Hybrid Live",
        },
    )


# =========================
# Generic RF Predict API
# =========================
...
# =========================
def open_browser():
    """Production dashboard auto-open."""
    time.sleep(8)  # wait for app + models + dashboard route
    url = "http://localhost:8000/dashboard"
    try:
        webbrowser.open_new(url)
        print(f"Dashboard: {url} (opened automatically)")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        ...
        host="0.0.0.0",
        port=8000,
        reload=False,  # keep auto-open reliable; reload can start twice
    )
